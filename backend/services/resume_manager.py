    ...
    if score >= 80:
        ...
        if tailor_result.get("status") in ("success", "warning"):
            ...
            with open(generated_pdf, "rb") as f:
                result["base64_file"] = base64.b64encode(
                    f.read()).decode("utf-8")

            # Note: We keep the file for now, cleanup happens in Task 3
        else:
            print(
                f"Tailoring failed: {tailor_result.get('error')}. Falling back to default.")
            with open(default_resume_path, "rb") as f:
                result["base64_file"] = base64.b64encode(
                    f.read()).decode("utf-8")
...
def score_resume(jd_text: str) -> int:
    ...
    profile_content = profile_path.read_text(
        encoding="utf-8") if profile_path.exists() else "Candidate Profile not found."
